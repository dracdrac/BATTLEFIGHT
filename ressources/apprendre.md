...
Ton deck représente les techniques de combats que ton personnage peut utiliser. 

Un deck est un paquet de 20 cartes qui contient 4 cartes pour chacun des 5 types.
...
Tri les cartes par types et crée deux decks en piochant 4 cartes par type pour chacun des deux decks.
...
Les decks sont méllangés et posés faces cachées pour former les pioches.
...
Lorsque la partie commence, on a aucune carte en main.
...
À ton tour tu joue une seule technique. Ça peut être une carte que tu joues depuis sa main ou une technique de base.
...
Les techniques de bases sont 5 techniques que tout le monde peut utiliser.

Ce sont des techniques de combats, au même titre que les cartes de ton deck, mais elles sont toujours accessibles pour tout le monde (pas besoin de les piocher).
...
Les étapes sont écrite avec des mots clés. La plus-part des mots clés sont plutôt transparents, en cas de doute, tous les mots clés sont expliqués dans le [glossaire](glossaire).
...
Quand tu déstabilise, choisi une carte en jeu qui a Stable et jette la dans la défausse de son propriétaire.

Si le **Stable** de la carte que tu souhaite déstabilisée est suivie d'un nombre, il te faudra "payer" en défaussant ce nombre de cartes de ta main. 
...
- Tu ne peux regagner que les points de vies perdus depuis ton dernier tour
...
**Défausser** est une condition. Elle te demande de défausser un certain nombre de cartes (depuis ta main vers ta défausse). C'est une condition donc si tu n'as pas assez de carte en main, tu ne peux pas executer la suite des étapes et la carte elle-même pars à la défausse
...
Pour rappel, quand tu déstabilise une carte **Stable**. Si le **Stable** en question est suivi d'un nombre, tu dois défausser autant de cartes (depuis ta main vers ta défausse). Tes points de **Charme** réduisent ce nombre.
